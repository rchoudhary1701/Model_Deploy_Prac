...
# (The simulate_data() function remains the same)
def simulate_data():
    ...
    df['sales'] = df['sales'].astype(int).clip(lower=10)
...
def train_and_save_models(bucket_name, sarima_path, xgboost_path):
    ...
    # Use the same paths for GCS blob names
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sarima-path', type=str, default='models/sarima_model.pkl')
    # --- THIS IS THE FIX ---
    parser.add_argument('--xgboost-path', type=str, default='models/xgboost_model.pkl')
